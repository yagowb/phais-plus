...
const risco = "Não foram realizados estudos em animais e nem em mulheres grávidas; ou então, os estudos em animais revelaram risco, mas não existem estudos disponíveis realizados em mulheres grávidas."
    ...
    <BaseLayout pageName='Detalhes do medicamento'>
      ...
      <div className="rounded-lg flex w-full flex-col bg-bg-main ml-4">
        ...
        <div className="w-full bg-bg-main mb-4 flex items-center">
          ...
          <img src={receitaTipo} alt="Sua Imagem" className="w-8 h-8 ml-4 mt-4" />
        </div>
        ...
        <div className="h-auto w-full flex bg-bg-main">
          <div style={{ fontSize: '14px' }} className="w-1/2 h-full text-14px">
            ...
            <div style={{ fontSize: '20px' }} className="mb-4 mt-2">{principioAtivo}</div>
            <div className='text-neutral-main' style={{ fontSize: '14px' }}>Grupos Farmacológicos</div>
            ...
            <div className='text-neutral-main' style={{ fontSize: '14px' }}>Indicações terapeuticas</div>
            <div style={{ fontSize: '18px' }} className="p-4">
              <ul className="list-disc pl-4">
            ...
            <div className='text-neutral-main' style={{ fontSize: '14px' }}>Risco na gravidez</div>
            ...
            <div style={{ fontSize: '14px' }}>
              Aprovado pela Anvisa: {dataAnvisa}<br />
              Receituário: {receita}
            </div>

            {/* Divisão esquerda */}
          </div>
          <div style={{ fontSize: '14px' }} className="w-1/4 h-full mb-4 mr-8 ml-8">
            ...
            <div className="bg-bg-main p-4">
              <ul className="bg-bg-layer rounded-lg divide-y divide-custom-divide">
                {lab.map((item, index) => (
                  <li key={index} className="text-16px py-4 mx-6">{item}</li>
                ))}
              </ul>
            </div>
              ...
              <ul className="bg-bg-layer rounded-lg divide-y divide-custom-divide">
                {genericos.map((generico, index) => (
                  <li key={index} className="py-4">
                    <div className="mx-6"> {/* Adiciona margem à esquerda e à direita */}
                      <span className="text-16px">{generico.nome}</span> {/* Nome em 16px */}
                    </div>
                    <div className="mx-6"> {/* Adiciona margem à esquerda e à direita */}
                      <span className="text-14px font-thin">{generico.fabricante}</span> {/* Fabricante em 14px e cinza */}
                    </div>
                  </li>
                ))}
              </ul>
            ...
            <div className="bg-bg-main p-4">
              <ul className="bg-bg-layer rounded-lg divide-y divide-custom-divide">
                {similares.map((similar, index) => (
                  <li key={index} className="py-4">
                    <div className="mx-6"> {/* Adiciona margem à esquerda e à direita */}
                      <span className="text-16px">{similar.nome}</span> {/* Nome em 16px */}
                    </div>
                    <div className="mx-6"> {/* Adiciona margem à esquerda e à direita */}
                      <span className="text-14px font-light">{similar.fabricante}</span> {/* Fabricante em 14px e cinza */}
                    </div>
                  </li>
                ))}
              </ul>
          ...
          <div className="p-2 w-1/4 bg-bg-layer max-h-100 rounded-lg mr-16 ml-8 mb-20">
            <div style={{ fontSize: '20px' }}><p className="text-center text-20px font-semibold mt-4 mb-6">APRESENTAÇÃO</p></div>
            <div style={{ fontSize: '16px' }} className="text-16px ml-2">Creme dematológico 1 MG/G</div>
            ...
            <div className="text-16px text-neutral-main ml-4">
              <ul className="list-disc pl-4">
                <li>Aplicar 1 camada fina sobre a área afetada, 1 vez ao dia</li>
              </ul>
            </div>
